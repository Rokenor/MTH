...
function progressiveMultiplication(
  ...
  if (array[index+1]) {
  	return progressiveMultiplication(item, array, index+1, prev ? array[index] * prev : array[index], result);
  }
...
function GetDivisorsANumber(number: number): number[] {
  ...
  const calculatedNumbers: number[] = simpleNumbers.reduce((result: any[], item, index) => {
    const shortList: number[] = simpleNumbers.filter((item, i) => i !== index);
